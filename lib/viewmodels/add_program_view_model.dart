...
class AddProgramViewModel extends ChangeNotifier {
  // ───────────────────────────────────────────── UI state
  final TextEditingController _dateController = TextEditingController(); // ✅ REMIS
  TextEditingController get dateController => _dateController; // ✅ REMIS
  ...
  // ✅ FONCTION : Réinitialiser le formulaire avec option de préserver la date
  void resetForm({bool preserveDate = false}) {
    ...
    _updateDateController(); // ✅ Met à jour le contrôleur dans tous les cas
    _isGenerating = false;
    notifyListeners();
  }
  ...
  Future<void> checkExistingProgram(BuildContext ctx) async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (dialogCtx) => AlertDialog(
        title: Text(tr('existing_program')),
        content: Text(tr('existing_program_message')),
        actions: [
          TextButton(
    ...
    if (confirm == true) {
      ...
      Navigator.of(ctx).pop(); // Ferme le popup de chargement

      resetForm(preserveDate: true); // ✅ Préserver la nouvelle date trouvée
    }
  }
  ...
  // ✅ FONCTION : Supprimer un exercice
  ...
  // ✅ MÉTHODE PRIVÉE : Mettre à jour le contrôleur de date
  void _updateDateController() {
    _dateController.text = "${selectedDate.day}/${selectedDate.month}/${selectedDate.year}";
  }

  // ✅ MÉTHODE PUBLIQUE : Mettre à jour la date sélectionnée
  ...
  Future<void> _autopickNextDate(BuildContext ctx) async {
    ...
    selectedDate = d;
    _updateDateController(); // ✅ CRITIQUE : Met à jour le contrôleur
    notifyListeners(); // ✅ CRITIQUE : Notifie les listeners
    
    ScaffoldMessenger.of(ctx).showSnackBar(SnackBar(
        content: Text(tr('new_date_selected', args: ["${d.day}/${d.month}/${d.year}"]))));
  }
  ...
  void dispose() {
    ...
    _dateController.dispose(); // ✅ CRITIQUE : Disposer le contrôleur de date
    super.dispose();
  }
